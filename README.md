...
```bash
make CUDA_ARCH=sm_89 HOST_CXX=/usr/bin/g++-13
```
...
The engine seeds each search batch with 256 bits from `/dev/urandom`. Every GPU
result is re-derived and pattern-checked independently on the CPU before it is
...
firmware's known-good private-key test vector. The opt-in GPU suite also runs a
short end-to-end CUDA search and independently verifies its output.
